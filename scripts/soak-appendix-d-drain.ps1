# Appendix D Deferred Drain Authority soak (ADR-0022 INV-NEG-018..022 / PR #99)
...
#
# Usage:
#   1. Install APK built from main (dd5bc2e+ / PR #99)
#   2. .\scripts\soak-appendix-d-drain.ps1 -ClearOnly
#   3. Repro WiFi flap on recovery peer; wait until EDGE_RECOVERED (~60-90s)
#   4. .\scripts\soak-appendix-d-drain.ps1 -CollectOnly
#   5. .\scripts\soak-appendix-d-drain.ps1 -AnalyzeOnly -Stamp <stamp>
#
# AnalyzeOnly delegates to analyze-appendix-d-drain.ps1
...
param(
    ...
    [switch]$AnalyzeOnly,
    [string]$Stamp = "",
    [string]$Prefix = "appendix-d-drain",
...
if ($ClearOnly) {
    ...
    Write-Host "Then: .\scripts\soak-appendix-d-drain.ps1 -CollectOnly"
    exit 0
}
...
if ($CollectOnly) {
    ...
    @(
        "Appendix D deferred drain soak"
    ...
    Write-Host "Analyze: .\scripts\soak-appendix-d-drain.ps1 -AnalyzeOnly -Stamp $Stamp"
    exit 0
}
...
if ($AnalyzeOnly) {
    ...
    & (Join-Path $PSScriptRoot "analyze-appendix-d-drain.ps1") -LogDir $outDir
    exit $LASTEXITCODE
}
