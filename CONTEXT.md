...
**Negotiation Capability Truth**:
某 edge 上 ICE restart **此刻是否可执行**的 level 真相：`probeIceRestartGate(edge).executable`（INV-NEG-012）。**不是**日志事件、**不是**执行许可。单观察 owner = **Coordinator**（INV-NEG-020）；Recovery 只消费、不产 Truth、不维护 observation ledger。
_Avoid_: NEGOTIATION_CAN_EXECUTE 日志 == permission, 用旧 STABLE observation 当当前 executable, Recovery 自建 capability ledger / 自产 CAN_EXECUTE

**Negotiation Capability Event**:
Capability Truth 的 **false→true rising-edge 通知**（`NEGOTIATION_CAN_EXECUTE`）。唤醒优化，不是 authority（INV-NEG-019）。Deferred execute 必须：只消费 **晚于该 intent 的 DEFER_ADMISSION baseline** 的 event；再 **re-probe** 当前 Truth；且 **当前 intent lineage 仍有效** 才可 execute。Coordinator 在枚举 negotiation seam **及 DEFER_ADMISSION 后立刻一次** 可 recompute（level 补偿）；**禁止** timer 轮询 / Recovery 自 schedule drain / mediaRestored 触发 CAN_EXECUTE（INV-NEG-020）。
_Avoid_: event == permission, 用 join 期 intentId=NONE 的 CAN_EXECUTE drain 后来的 R1/R2, 丢 event 就永久 WAIT 且无合法 seam 再 probe, 每 N 秒 blind retry

**ICE Restart Intent**:
某 Recovery Attempt 上「需要执行 bounded ICE restart」的未完成意图；settling 导致 DEFERRED 时必须保留，否则 watchdog 会误判 media failure。Intent 由创建它的 **Obligation Episode / Edge Recovery Controller** 拥有（INV-NEG-002 / **INV-NEG-018**）——**单 lifecycle authority** 独占 create / retain / execute / expire / cancel；实现可拆 helper，authority 不拆。不得在 obligation CLOSED 后另存独立 intent（拒 R-2）。Negotiation / Coordinator **只产 capability fact**（如 `NEGOTIATION_CAN_EXECUTE`），**MUST NOT** mutate intent lifecycle（Capability ≠ Ownership）。Intent **不得**跨 SUPERSEDE / obligation CLOSE / generation 存活（INV-NEG-003）；**SUPERSEDE = lineage cut**（predecessor → STALE_DISCARD/EXPIRED，永不被 late answer / capability / transport 复活；successor **新建** intent + **新** DEFER_ADMISSION baseline，不继承 predecessor freshness/wakeup — **INV-NEG-022**）。失效须进入可审计的 terminal，不得仅静默清空字段。Deferred 时带 **domain**（ICE restart gate defer → NEGOTIATION）；completion evidence 必须覆盖该 domain 才可随 obligation 关闭（INV-REC-026）。Drain 只操作 **当前 active slot lineage**（intentId + attemptId + obligationGen）。
_Avoid_: intent == 已 dispatch / iceRestartIssued, 把 intent 放进 PeerConnection settling 状态机, 跨 obligationGeneration 继承 pending restart, close 后仍等 NEGOTIATION_RELEASED 执行, successor 继承 predecessor deferred restart / baseline / wakeup, 关义务后保留 orphan intent, Coordinator 存 deferred intent / Negotiation retry queue, ICE_CONNECTED 或 mediaRestored 自动 execute deferred intent, UI/Presence 决策 deferred intent, execute 归属 Negotiation 形成第二生命周期, late event 复活 STALE_DISCARD

**Deferred Intent Domain**:
Deferred intent 阻塞所在的恢复子域；决定何种 completion evidence 有权覆盖/关闭它。当前至少：`NEGOTIATION`（gate settling）、`MEDIA`、`TRANSPORT`、`CONTROL`；`SESSION`/`ALL` 仅生命周期终结类 evidence。见 ADR-0022 INV-REC-026。**Cardinality（Q12 M-1 / Drain Q5）：** 每个 Obligation Episode / 当前 attempt **至多一个** active deferred intent slot；drain = **current-slot-only**（INV-NEG-022）。
_Avoid_: 所有 deferred 共用一个 RECOVERED 桶, ICE_CONNECTED 覆盖 NEGOTIATION, 多 intent 队列按旧 intentId 路由 late event, 本刀引入 per-domain sub-obligation
...
Deferred intent 的动作已被 dispatch / attempt 消化（如 ICE restart `createOffer` 路径已发出）。解消的是 **intent action**（deferral 可清；Intent lifecycle: deferred→executed），不是 Obligation Episode（Recovery lifecycle: open→recovered）。见 INV-REC-031 / Q14 C-3 / INV-NEG-016 / **INV-NEG-021**。**禁止** `EXECUTED → RECOVERED` 直达；义务关闭仍须既有 Completion Authority 的 **post-`restartDispatchAt` Restart-Resolved Evidence**。Drain Authority **不得**另造 completion evidence 集合。
_Avoid_: EXECUTED == RECOVERED, EXECUTED 后复用 pre-dispatch mediaRestored 关义务, dispatch 成功即 closeObligation, Drain-only canClose 第二套 authority
...
`RECOVERY_REATTACH` 控制面消息的 lineage-scoped 送达证明层；区分「peer 是否收到 recovery offer」与「negotiation / completion 是否完成」。**Delivery fact producer** 报 `DELIVERY_PENDING` / `DELIVERY_CONFIRMED` / `DELIVERY_EXHAUSTED`；**Episode Owner** 拥有 bounded retransmit policy 与 scheduling；**Coordinator** 仅执行同 lineage dispatch；**Transport** 仅 I/O 事实。**`DELIVERY_CONFIRMED` 唯一权威** = 匹配当前 identity 的 `RECOVERY_REATTACH_ACK`（非 SEND accept、`LINK_READY`、heartbeat、ICE）。`DELIVERY_EXHAUSTED` 关闭 delivery budget，**不**关闭 recovery / obligation。重投仅 `deliveryAttemptId++`；禁止 delivery retry 新建 `offerLineageId` / obligation / ICE restart。见 ADR-0035 + Appendix A（PR2）。
_Avoid_: Transport-owned retry, SEND 当 peer 已收到, DELIVERY_CONFIRMED→RECOVERED, exhaustion→auto supersede, delivery retry 刷新 RECOVERY_WATCHDOG

**Conference Presence Projection**:
会议在场读模型：本端可见的 **joined / connected / recovering** 人数与 peer 集合，由 `ConferencePresenceProjector` 从 Membership roster、`EdgeRecoveryFacts`、`ConnectedPeers` 等只读事实产出。UI **MUST** 只消费此投影（及 Runtime 投影的 phase），**MUST NOT** 读 `ReachabilitySnapshot` 或在 ViewModel 内重建 presence。见 ADR-0022 R27′。**Presence Projection / Recovery UX：UVCP CLOSED** — [ADR-0034](docs/adr/0034-user-visible-connectivity-projection.md) **IMPLEMENTED**；**G-PRES-A PASS**；media-fact **PR #98 PASS**；**G-PRES-E BLOCKED_BY_COMPLETION**（长 SYNCING = 准确投影，不是 PRES 缺陷）。验收：PR #97 架构 PASS + PR #98 media fact PASS。禁止再开 PRES/SIG/B3 grill。**Negotiation Deferred Drain Authority：CLOSED / IMPLEMENTED**（ADR-0022 Appendix D；grill Q1–Q5；INV-NEG-018..022；PR #99；禁扩 Q6 / 禁改 UVCP/timeout）。**Recovery Signaling Delivery Assurance：ADR-0035 ACCEPTED**（[0035](docs/adr/0035-recovery-scoped-delivery-assurance.md)；INV-DELIVERY-001..007；**PR1 Observability PASS / CLOSED**（2026-07-29）；**PR2 Grill CLOSED**（2026-07-29；Appendix A）；下一步=**PR2 implementation**；禁碰 UVCP/Drain/Completion）。**禁止**因 SYNCING≠RECONNECTING 重开 UVCP；**禁止**无 RECEIVE 时查 answer/completion；先分类再 ADR。异常先分层（domain → projection → admission）。禁止 Projection timeout / UI→completion|repair|admission 反向耦合。
_Avoid_: memberKeys.size 当 joinedCount, ICE/transport 推断 recovering, 扩 ConferenceRuntimeProjector 塞 presence 字段, sessionEdgeRecovering==reconnecting, Projection 10s 强制 CONNECTED, 因 SYNCING 久开 UI bug, RECOVERED 强制改 UI, Projection closeObligation, 重开 PRES/SIG/B3 grill, 把长 SYNCING 当 UVCP bug, 用 Projection timeout 关 CONNECTED, Case1 重开 readiness/qualification/PRR, 未分类就写 Offer Delivery ADR, 把 SENT 当 peer 已收到, 在 D1 上先查 answer, 因 M02 SYNC/M01 RECONNECT 重开 UVCP, 用 group REMOTE_RECEIVE 证明 recovery L* 已送达

**User Visible Connectivity Projection**:
面向用户的连通语义投影（Presence Projection / Recovery UX）：把各 domain 的只读事实解释成用户该看到的状态。**双轴映射（Q6=F-4）**：Axis A=`MEDIA_USABILITY`（`MEDIA_OK` / `MEDIA_UNAVAILABLE`）；Axis B=`CONTROL_SYNC_STATE`（`STABLE` / `SYNCING` / `DEGRADED`）；`(Media, ControlSync) → UserVisibleConnectivityState`。硬否决等价 F-1：`MEDIA_OK` **禁止**产出 `RECONNECTING`。**输出粒度 = peer-level**（真相源）；Meeting pill / 摘要仅为二级聚合消费者。**不是** truth owner；不得 mutate 任何 owner。可读取 media / peer-edge readiness / negotiation sync **coarse**；lifecycle（obligation / `sessionEdgeRecovering` / phase）仅诊断，**不得**直接当 UI 连通态。**MUST NOT** 拥有 timer、expiration、retry 或 terminal transition（**Q5=L-1**）。**显示与 admission 分离（Q9=C-2+C-4）**：Projection 只驱动 pill/avatar/hint；硬门控仍由 domain admission；Projection **MAY** soft hint，**MUST NOT** grant/deny protocol actions。Grill **Q1–Q9 ACCEPTED**；规范见 **[ADR-0034](docs/adr/0034-user-visible-connectivity-projection.md)**；**G-PRES-A PASS**（2026-07-29）；**UVCP CLOSED**；media-fact **PR #98 PASS**；**G-PRES-E BLOCKED_BY_COMPLETION** — Deferred Drain **CLOSED / IMPLEMENTED**（[ADR-0022 Appendix D](docs/adr/0022-recovery-completion-ownership.md)；PR #99）。**Recovery Signaling Delivery Assurance** — ADR-0035 **ACCEPTED**；PR1 **CLOSED**；PR2 grill **CLOSED**（[investigation](docs/investigations/ice-restart-offer-delivery-investigation.md)）。媒体恢复 + control pending → `SYNCING` 为支持的中间态；长 SYNCING ≠ PRES bug。
_Avoid_: 用 sessionEdgeRecovering 当 reconnecting, 把 obligation OPEN 直接显示为对方断线, 在 projection 里 closeObligation / markRecovered / rebind, 把 SIGNALING_NOT_STABLE / HAVE_LOCAL_OFFER / deferredReason 原文露给 UI, Meeting pill 反向当 truth, recoveringPeers 当 UI 连通语义输入, negotiation pending alone → RECONNECTING, DEGRADED 折叠成 SYNCING, count-based escalation, Projection 当 admission, SYNCING 禁用呼叫, CONNECTED 绕过 admission, Projection timeout 强制 CONNECTED, Case1 重开 readiness/qualification/PRR
